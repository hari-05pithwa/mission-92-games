...
//   { id: 4, img: "/images/02 Das_bhav.png", label: "04", color: "bg-[#9B59B6]" },
// ];
...
//   4: "संभूय प्रत्यहं कार्या गृहसभा गृहस्थितैः। कर्तव्यं भजनं गोष्ठिः शास्त्रपाठादि तत्र च॥ ८६॥",
// };
...
// Fixed IDs and unique keys for each card
const CARDS = [
  { id: 1, img: "/images/01 Vachanamrut.png", label: "01", color: "bg-[#FF6B6B]" },
  { id: 2, img: "/images/03 Divy_bhav.png", label: "02", color: "bg-[#4D96FF]" },
  { id: 3, img: "/images/04 Ghar_sabha.png", label: "03", color: "bg-[#9B59B6]" },
  { id: 4, img: "/images/02 Das_bhav.png", label: "04", color: "bg-[#6BCB77]" }, // Unique ID and color
];

const ANSWERS = {
  1: "स्वामिवार्ताः पठेन्नित्यं तथैव वचनामृतम्। गुणातीतगुरूणां च चरितं भावतः पठेत्॥ २३६॥",
  2: "सत्सङ्गिषु सुहृद्भावो दिव्यभावस्तथैव च। अक्षरब्रह्मभावश्च विधातव्यो मुमुक्षुणा॥ १४०॥",
  3: "संभूय प्रत्यहं कार्या गृहसभा गृहस्थितैः। कर्तव्यं भजनं गोष्ठिः शास्त्रपाठादि तत्र च॥ ८६॥",
  4: "आदरेण प्रणामैश्च मधुरवचनादिभिः। यथोचितं हि सम्मान्या वृद्धा ज्ञानवयोगुणैः॥ १५९॥",
};
...
export default function PlayfulRevealPage() {
  ...
  const triggerCelebration = () => {
    const end = Date.now() + 1500;
    const frame = () => {
      confetti({ particleCount: 4, angle: 60, spread: 60, origin: { x: 0, y: 0.8 }, colors: ["#FF6B6B", "#4D96FF", "#9B59B6", "#6BCB77"] });
      confetti({ particleCount: 4, angle: 120, spread: 60, origin: { x: 1, y: 0.8 }, colors: ["#FF6B6B", "#4D96FF", "#9B59B6", "#6BCB77"] });
      if (Date.now() < end) requestRef.current = requestAnimationFrame(frame);
    };
    frame();
  };
  ...
  return (
    <div className="h-screen w-screen bg-[#FFFDF0] flex flex-col items-center overflow-hidden p-12 relative font-sans">
      ...
      {/* Changed grid-cols-3 to grid-cols-4 to fit all cards in one row */}
      <div className="flex-1 w-full max-w-6xl grid grid-cols-4 gap-8 items-center px-4">
        {CARDS.map((card, index) => {
          const isOpened = openedIds.has(card.id);
          return (
            <motion.div
              ...
              `}
            >
              <span className={`text-8xl font-black italic mb-2 relative z-10 select-none transition-all duration-300
                ${isOpened ? "text-slate-300 opacity-20 scale-75" : "text-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.15)] group-hover:scale-110"}
              `}>
                {card.label}
              </span>
